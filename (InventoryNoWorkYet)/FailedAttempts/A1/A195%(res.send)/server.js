...
app.post("/process_form", function (request, response, next) {
    ...
    for (i = 0; i < products.length; i++) {
        ...
        if (isNonNegativeInteger(quantity) == false) {
            errors['quantity' + i] = response.send(`Please enter valid quantities for ${products[i].name}`);
        } // INITIALLY there was no response.send
        if (quantity > 0) {
            ...
            if (quantity > products[i].quantity_available) { // Error alert if quantity submitted is more than available in inventories.
                errors['quantity' + i] = response.send(`${quantity} of ${products[i].name} not available. Only ${products[i].quantity_available} available.`);
            } // INITIALLY there was no response.send
        }
    }
