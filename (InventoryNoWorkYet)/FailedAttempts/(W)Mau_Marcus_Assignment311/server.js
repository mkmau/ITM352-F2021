...
app.post("/checkout", function (request, response) {
    ...
    // Info of sender
    var user_email = 'noreply@galorestore.com';
    var mailOptions = {
        from: 'GaloreStore',
        to: user_email,
        subject: 'Your Order from CDs Galore!',
        html: invoice_str
    };

    transporter.sendMail(mailOptions, function (error, info) {
        if (error) { // If there are errors in sending invoice (e.g., due to network issue), display error message below invoice
            invoice_str += `<p>There was an error and your invoice was not sent!</p> <p>Return to <a href="/index.html">logout</p>`;
        } else { // Otherwise, show that the email was sent successfully
            invoice_str += '<p>The invoice was sent to your email. Your CDs will be at your doorstep in one week!</p> <p>Return to <a href="/index.html">logout</p>';
        }
